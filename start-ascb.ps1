...
if (-not $backendReady) {
    ...
    # Start backend in dev mode
        $backendProcess = Start-Process -FilePath "cmd" -ArgumentList "/k", "cd", "/d", $backendDir, "&&", "mvn", "spring-boot:run", "-q", "-DskipTests", "-Dspring-boot.run.profiles=dev" -NoNewWindow -PassThru
