...
function Start-Application {
    ...
    if (-not $backendReady) {
        ...
        $backendProcess = Start-Process -FilePath cmd.exe -ArgumentList "/c", "cd", "/d", "`"$backendDir`"", "&&", "mvn", "spring-boot:run", "-Dspring.profiles.active=dev", "-q" -WindowStyle Hidden -PassThru
